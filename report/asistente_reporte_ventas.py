...
class AsistenteReporteVentas(models.TransientModel):
    _name = 'l10n_sv.asistente_reporte_ventas'
    ...
    def print_report_contribuyente(self):
        ...
        data = {
             'ids': [],
             'model': 'l10n_sv.asistente_reporte_ventas',
             'form': self.read()[0]
        }
        return self.env.ref('l10n_sv.action_reporte_ventas').report_action(self, data=data)
    ...
    def print_report_excel(self):
        for w in self:
            ...
            res = self.env['report.l10n_sv.reporte_ventas'].lineas(dict)
            lineas = res['lineas']
            totales = res['totales']
            libro = xlwt.Workbook()
        ...
        return {
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'l10n_sv.asistente_reporte_ventas',
            'res_id': self.id,
            'view_id': False,
