...
class AsistenteReporteCompras(models.TransientModel):
    _name = 'l10n_sv.asistente_reporte_compras'
    ...
    def print_report(self):
        data = {
             'ids': [],
             'model': 'l10n_sv.asistente_reporte_compras',
             'form': self.read()[0]
        }
        return self.env.ref('l10n_sv.action_reporte_compras').report_action(self, data=data)
    ...
    def print_report_excel(self):
        for w in self:
            ...
            res = self.env['report.l10n_sv.reporte_compras'].lineas(dict)
            lineas = res['lineas']
            totales = res['totales']
            libro = xlwt.Workbook()
        ...
        return {
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'l10n_sv.asistente_reporte_compras',
            'res_id': self.id,
            'view_id': False,
